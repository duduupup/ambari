...
def setup_python():
  ...
  if not os.path.isfile('/opt/rh/python27/enable'):
    Directory(format("{pkgs_cache_path}"), not_if=format("test -d {pkgs_cache_path}"), owner='root', group='root',
              create_parents=True, mode=0755)
    Execute("yum install -y python27-python python27-python-debug python27-python-debuginfo "
            "python27-python-devel python27-python-pip")
    # packages_server = params.config["clusterHostInfo"]["ambari_server_host"][0]
    # Execute(format("scp -r root@{packages_server}:{xsetup_path}/soft/rds_ext/python-2.7.zip {pkgs_cache_path}"), user='root')
    base_url = json.loads(params.config['hostLevelParams']['repo_info'])[0]['baseUrl']
    Execute(format("rm -f {pkgs_cache_path}/python-2.7.zip"), user='root')
    Execute(format("wget -q -O {pkgs_cache_path}/python-2.7.zip {base_url}/python/2.7.13/python-2.7.zip"), user='root')
    Execute(format("rm -rf /opt/rh/python27 && unzip -q {pkgs_cache_path}/python-2.7.zip -d /opt/rh"), user='root')
